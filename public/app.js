...
class NodeDeploymentPanel {
    ...
    async stopProject(projectPath) {
        ...
        const processId = Array.from(this.runningProcesses.entries()).find(([id, process]) => 
            process.projectPath === projectPath
        )?.[0];
    ...
    async createNewProject() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                await this.createProjectFiles(projectName, mainFile);
    ...
    async createProjectFiles(projectName, mainFile) {
        // This would create basic project structure
    ...
    getTotalProjectsCount() {
        // This would count total project directories
        return this.getProjectDirectories().then(projects => projects.length);
    }
    ...
    startAutoRefresh() {
        ...
        // Refresh running processes every 10 seconds
        setInterval(() => {
            this.loadRunningProcesses();
        }, 10000);
    }
    ...
    refreshSystemInfo() {
        this.loadSystemInfo();
        this.showToast('System info refreshed', 'info');
    }
}
...
function showCreateFolderModal() {
    window.app.showCreateFolderModal();
}

function showUploadFileModal() {
    window.app.showUploadFileModal();
}
...
function clearConsole() {
    window.app.clearConsole();
}

function downloadLogs() {
    window.app.downloadLogs();
}
...
function navigateToDirectory(path) {
    window.app.navigateToDirectory(path);
}
...
function deleteFile(path) {
    window.app.deleteFile(path);
}

// Project management functions
function runProject(path) {
    window.app.runProject(path);
}

function stopProject(path) {
    window.app.stopProject(path);
}
...
function openProjectFolder(path) {
    window.app.openProjectFolder(path);
}